...
router.get('/facility/:facilityId', async (req, res) => {
  try {
    const facility = await prisma.facility.findUnique({
      where: { id: (req.params as { facilityId: string }).facilityId },
      select: { waiverRequired: true, waiverText: true, waiverVersion: true },
    });
    if (!facility || !facility.waiverRequired || !facility.waiverText) {
      return res
        .status(404)
        .json({ error: 'No waiver found for this facility' });
    }
    res.json({
      waiverRequired: facility.waiverRequired,
      waiverText: facility.waiverText,
...
router.post('/sign', optionalAuthMiddleware, async (req, res) => {
  try {
    ...
    if (!facility || !facility.waiverRequired || !facility.waiverVersion) {
      return res
        .status(400)
        .json({ error: 'This facility does not have a waiver' });
    }
    ...
    if (existing)
      return res
        .status(409)
        .json({ error: 'Waiver already signed for this version' });
