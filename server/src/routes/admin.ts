...
router.get('/verifications/pending', authMiddleware, async (req, res) => {
  try {
...
// Review verification
router.put('/verifications/:id/review', authMiddleware, async (req, res) => {
  try {
    const userId = req.user!.userId;
    const user = await prisma.user.findUnique({
      where: { id: userId },
    ...
    res
      .status(500)
      .json({ error: error.message || 'Failed to review verification' });
  }
});
