...
import type { Conversation, Message, MessagesPage, MessageReaction } from '../../types/messaging';
...
class ConversationServiceClass extends BaseApiService {
  ...
  async getConversations(type?: string): Promise<Conversation[]> {
    ...
    return this.get<Conversation[]>(API_ENDPOINTS.CONVERSATIONS.BASE, { params });
  }
  ...
  async getMessages(id: string, cursor?: string): Promise<MessagesPage> {
    ...
    return this.get<MessagesPage>(API_ENDPOINTS.CONVERSATIONS.MESSAGES(id), { params });
  }

  async sendMessage(id: string, content: string, replyToId?: string): Promise<Message> {
    const body: Record<string, string> = { content };
    if (replyToId) body.replyToId = replyToId;
    return this.post<Message>(API_ENDPOINTS.CONVERSATIONS.SEND_MESSAGE(id), body);
  }
  ...
  async pinMessage(conversationId: string, messageId: string): Promise<void> {
    return this.post<void>(API_ENDPOINTS.CONVERSATIONS.PIN(conversationId, messageId), {});
  }
  ...
  async getUnreadCount(): Promise<number> {
    const res = await this.get<{ count: number }>(API_ENDPOINTS.CONVERSATIONS.UNREAD_COUNT);
    return res.count;
  }
  ...
  async addReaction(messageId: string, emoji: string): Promise<MessageReaction> {
    return this.post<MessageReaction>(API_ENDPOINTS.MESSAGES.REACTIONS(messageId), { emoji });
  }

  async removeReaction(messageId: string, emoji: string): Promise<void> {
    return this.delete<void>(API_ENDPOINTS.MESSAGES.REMOVE_REACTION(messageId, emoji));
  }
}
