          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.body}>
              {Array.from(grouped.entries()).map(([courtId, dateMap]) => {
                const courtName = dateMap.values().next().value?.[0]?.courtName ?? '';
                return (
                  <View key={courtId} style={styles.courtGroup}>
                    <View style={styles.courtHeader}>
                      <Ionicons name="basketball" size={16} color={colors.cobalt} />
                        ...
                        {slots
                          ...
                          .map((slot) => (
                            <View key={slot.slotId} style={styles.slotRow}>
                              ...
                              <Text style={styles.slotPrice}>${slot.price.toFixed(2)}</Text>
                            </View>
                          ))}
                      </View>
                    ))}
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontFamily: fonts.body, ...typeScale.bodySm, color: colors.inkFaint, marginTop: 4 },
  body: { padding: Spacing.lg },
  courtGroup: { marginBottom: Spacing.lg },
  courtHeader: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: Spacing.sm },
  courtName: { fontFamily: fonts.semibold, ...typeScale.body, color: colors.ink },
  ...
  slotPrice: { fontFamily: fonts.semibold, ...typeScale.body, color: colors.cobalt },
  totalRow: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    padding: Spacing.md, backgroundColor: `${colors.cobalt}10`, borderRadius: 12,
    marginHorizontal: Spacing.lg, marginBottom: Spacing.md,
  },
  totalLabel: { fontFamily: fonts.semibold, ...typeScale.h3, color: colors.ink },
  totalPrice: { fontFamily: fonts.heading, ...typeScale.h2, color: colors.cobalt },
  insuranceSection: {
