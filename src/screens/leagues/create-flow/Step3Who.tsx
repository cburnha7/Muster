...
function buildYearOptions(): SelectOption[] {
  ...
  const opts: SelectOption[] = [{ label: 'No limit', value: '' }];
...
function ageFromBirthYear(year: string): number | null {
  if (!year) return null;
...
export function Step3Who() {
  ...
  const computedAge = ageFromBirthYear(state.maxBirthYear);
  const privateSelected = state.visibility === 'private';
  const publicSelected = state.visibility === 'public';
  const requiredTeams = parseInt(state.numberOfTeams) || 0;
  const invitedCount = state.invitedRosters.length;
  ...
  return (
    ...
    >
      ...
      <TextInput
        ...
        onChangeText={v =>
          dispatch({ type: 'SET_FIELD', field: 'numberOfTeams', value: v })
        }
      />
      ...
      <View style={styles.row}>
        <View style={styles.flex2}>
          <FormSelect
            ...
            value={state.maxBirthYear}
            options={yearOptions}
            onValueChange={v =>
              dispatch({ type: 'SET_FIELD', field: 'maxBirthYear', value: v })
            }
          />
        </View>
        {computedAge !== null && (
