    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.settingItem}>
          ...
          <Switch value={darkMode} onValueChange={setDarkMode} trackColor={{ false: colors.surfaceContainerHigh, true: colors.secondaryContainer }} thumbColor={darkMode ? colors.secondary : colors.surfaceContainerLow} />
        </View>
        <View style={styles.settingItem}>
          <Text style={styles.settingLabel}>Location Services</Text>
          <Switch value={locationServices} onValueChange={setLocationServices} trackColor={{ false: colors.surfaceContainerHigh, true: colors.secondaryContainer }} thumbColor={locationServices ? colors.secondary : colors.surfaceContainerLow} />
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.menuItem}><Text style={styles.menuItemText}>Privacy Policy</Text><Ionicons name="chevron-forward" size={18} color={colors.outline} /></TouchableOpacity>
        <TouchableOpacity style={styles.menuItem}><Text style={styles.menuItemText}>Terms of Service</Text><Ionicons name="chevron-forward" size={18} color={colors.outline} /></TouchableOpacity>
        <TouchableOpacity style={styles.menuItem} onPress={handleExportData}><Text style={styles.menuItemText}>Export My Data</Text><Ionicons name="chevron-forward" size={18} color={colors.outline} /></TouchableOpacity>
        {!isDependent && (
          <TouchableOpacity style={styles.menuItem} onPress={() => setShowDependantsModal(true)}>
            <Text style={styles.menuItemText}>Manage Dependants</Text><Ionicons name="chevron-forward" size={18} color={colors.outline} />
          </TouchableOpacity>
        )}
        <TouchableOpacity style={styles.menuItem} onPress={handleLogout}><Text style={[styles.menuItemText, { color: colors.tertiary }]}>Logout</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.menuItem, { borderBottomWidth: 0 }]} onPress={handleDeleteAccount} disabled={deleting}>
          <Text style={[styles.menuItemText, { color: colors.error }]}>{deleting ? 'Deleting...' : 'Delete Account'}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.menuItem}><Text style={styles.menuItemText}>Help & Support</Text><Ionicons name="chevron-forward" size={18} color={colors.outline} /></TouchableOpacity>
        <TouchableOpacity style={[styles.menuItem, { borderBottomWidth: 0 }]}><Text style={styles.menuItemText}>Rate the App</Text><Ionicons name="chevron-forward" size={18} color={colors.outline} /></TouchableOpacity>
      </View>
        ...
        <Pressable style={styles.modalBackdrop} onPress={() => { setShowDependantsModal(false); setSelectedDep(null); }}>
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => { setShowDependantsModal(false); setSelectedDep(null); }}><Ionicons name="close" size={22} color={colors.onSurface} /></TouchableOpacity>
            </View>
                ...
                dependents.map((dep) => (
                  <View key={dep.id}>
                    <TouchableOpacity style={styles.depRow} onPress={() => setSelectedDep(selectedDep?.id === dep.id ? null : dep)} activeOpacity={0.7}>
                      ...
                      <Ionicons name={selectedDep?.id === dep.id ? 'chevron-up' : 'chevron-down'} size={18} color={colors.outline} />
                    </TouchableOpacity>
                    {selectedDep?.id === dep.id && (
                      <View style={styles.depActions}>
                        <TouchableOpacity style={styles.depActionBtn} onPress={() => { setShowDependantsModal(false); setSelectedDep(null); (navigation as any).navigate('DependentProfile', { dependentId: dep.id }); }}>
                          ...
                          <Text style={[styles.depActionText, { color: colors.primary }]}>Manage</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.depActionBtn} onPress={() => handleDeleteDependant(dep)}>
                          <Ionicons name="trash-outline" size={16} color={colors.error} />
                          <Text style={[styles.depActionText, { color: colors.error }]}>Delete</Text>
                        </TouchableOpacity>
                      </View>
                    )}
                  </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontFamily: fonts.heading, fontSize: 18, color: colors.onSurface, marginBottom: 12 },
  settingItem: {
    flexDirection: 'row',
  ...
  settingLabel: { fontFamily: fonts.body, fontSize: 16, color: colors.onSurface },
  menuItem: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  menuItemText: { fontFamily: fonts.body, fontSize: 16, color: colors.onSurface },
  infoValue: { fontFamily: fonts.body, fontSize: 16, color: colors.outline },
  // Modal
  modalBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.35)', justifyContent: 'center', paddingHorizontal: 20, paddingBottom: 70 },
  modalCard: { backgroundColor: colors.surfaceContainerLowest, borderRadius: 16, maxHeight: '80%', overflow: 'hidden', shadowColor: colors.onSurface, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.15, shadowRadius: 20, elevation: 10 },
  modalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: colors.outlineVariant },
  modalTitle: { fontFamily: fonts.heading, fontSize: 20, color: colors.onSurface },
  modalScroll: { paddingHorizontal: 16, paddingVertical: 8 },
  emptyText: { fontFamily: fonts.body, fontSize: 15, color: colors.outline, textAlign: 'center', paddingVertical: 20 },
  depRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, gap: 10, borderBottomWidth: 1, borderBottomColor: colors.outlineVariant },
  depAvatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center' },
  depAvatarText: { fontFamily: fonts.ui, fontSize: 16, color: colors.surfaceContainerLowest },
  depName: { flex: 1, fontFamily: fonts.body, fontSize: 16, color: colors.onSurface },
  ...
  addDepBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 14, borderTopWidth: 1, borderTopColor: colors.outlineVariant },
  addDepText: { fontFamily: fonts.ui, fontSize: 15, color: colors.primary },
});
